--[[
    ...
    Modified by:
        Robote1122
    Updated with Key System (Pure API - No hardcoded URL/Key)
]]
...
-- ============================================
-- СИСТЕМА КЛЮЧЕЙ (ВНЕ ТАБЛИЦЫ)
-- ============================================

function xlp.checkKey(apiUrl, key, userId)
    ...
    local success, response = pcall(function()
        return (syn and syn.request or http_request or http.request or request)({
            Url = apiUrl .. "/api/keys/check",
            Method = "POST",
            Headers = {
                ["Content-Type"] = "application/json"
            },
...
function xlp.activateKey(apiUrl, key, userId)
    userId = userId or game.Players.LocalPlayer.UserId
...
function xlp.validateAndActivate(apiUrl, key, userId)
    userId = userId or game.Players.LocalPlayer.UserId

    local checkResult = xlp.checkKey(apiUrl, key, userId)

    if not checkResult then
        return false
    end

    if checkResult.detail then
        return false
    end
    ...
    local activateResult = xlp.activateKey(apiUrl, key, userId)
